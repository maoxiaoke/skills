...
6. If the export script fails (no JS runtime found), tell the user:
   > "No JavaScript runtime found. Install Node.js (https://nodejs.org), or use bun/deno, then run export again."
...
`export.sh` renders each size with the vendored **resvg-wasm** renderer (`scripts/svg2png.mjs`) — no external converter (Inkscape/librsvg/sharp) and no package install required, just a JS runtime (node, bun, or deno). The renderer is zero-install: only the wasm binary (`scripts/index_bg.wasm`) and bindings (`scripts/resvg.mjs`) are bundled.

**No fonts are shipped.** Icons (pure vector) and logos whose text is converted to `<path>` need no fonts at all. For logos that keep live `<text>`, the renderer scans the OS font directories for the referenced family and feeds it to resvg — so common system fonts (Helvetica, Arial, Georgia…) work automatically. If a referenced family isn't on the system, it's reported on stderr; in that case use a common system font or convert the text to a path. This is why the SVG conventions above call for system fonts or `<path>` text.
