...
## Company Brand Mode (SAVANA)

This skill ships with the company brand kit: [BRAND_GUIDELINES.md](BRAND_GUIDELINES.md) (design tokens distilled from the SAVANA Brand Bible 2026) and [assets/logo.svg](assets/logo.svg) (the "savana" wordmark).

**Activate Brand Mode when** the deck is company-facing: the user mentions SAVANA, the company, official/internal/company presentations, brand reviews, or business content clearly belonging to the company. If unsure whether a work deck should be on-brand, include it in the Phase 1 questions (header: "Brand", options: "Official SAVANA branding" / "Free style"). Personal or clearly non-company decks skip Brand Mode entirely — nothing changes for them.

**When Brand Mode is active:**

- Read `BRAND_GUIDELINES.md` before Phase 2. Its rules **override** the Design Aesthetics section above (fonts, palette, and voice are mandated, not creative choices).
- The preset and bold-template libraries stay available as **layout sources**: their grid logic, composition, spacing rhythm, and component grammar are all compatible with the brand — only their skin (colors, fonts, voice) must be replaced with brand tokens. Shortlist templates by `mood`/`density`/`formality` as usual.
- Phase 2 previews: 1 strict brand style (Sunshine Yellow + Midnight Blue core combo, by the book) + 2 brand-skinned variations — each may be a preset/bold-template layout re-skinned with brand tokens, or a free brand-flavored design. All three carry the official palette, fonts, and logo; do not offer off-brand options.
- Phase 3 must apply the brand tokens throughout: official colors with WCAG-safe pairings only, Montserrat + Poppins, the inlined logo on every slide (bottom-left, recolored per background — see logo rules), sentence-case copy that respects the banned-word list, and the standard copyright footer on the closing slide.
...
**Brand Mode:** if active, all 3 previews are built on `BRAND_GUIDELINES.md` tokens (1 strict brand style + 2 brand-skinned variations). Presets and bold templates may still supply the layout for the variations — shortlist via the selection index as usual, then re-skin with the brand palette, fonts, and voice. Embed the inlined logo bottom-left in each preview, then continue at Step 2.1.
...
- **Brand Mode exception:** preserve only the layout system (spacing rhythm, grid logic, component grammar, decorative vocabulary where it doesn't clash with logo/palette rules); fonts, colors, and voice always come from `BRAND_GUIDELINES.md`.
...
- **Brand Mode:** apply every rule in `BRAND_GUIDELINES.md` — official palette with WCAG-safe pairings, Montserrat + Poppins, inlined logo bottom-left on every slide (CSS-recolored to the approved variant for each background), sentence-case copy with no banned words, copyright footer on the closing slide. The logo SVG travels inline in the HTML, so the deck stays a single self-contained file.
...
After delivery, **ask the user:** _"Would you like to share this presentation? I can upload it to a live URL (works on any device including phones) or export it as a PDF."_

Options:

- **Upload to URL** — Shareable link that works on any device
...
### 6A: Upload to a Live URL (BaaS Storage)

This uploads the presentation to BaaS Storage via the **`baas-upload` skill** and returns a permanent link (`https://baas-store.mayfair-inc.com/r/{root_id}/`) that works on any device (phones, tablets, laptops).

1. **Check prerequisites** — The `baas-upload` skill requires a valid `melo-cli` login (token stored under `~/.config/melo-cli/`). If the upload fails with an auth error, ask the user to run `melo-cli login` first, then retry.

2. **Choose the upload mode:**
   - **Single self-contained HTML** (no local image/video references — the normal output of this skill): use the skill's `html` mode. It serves the file as a static site (internally renamed to `index.html`; the original file is not modified).
   - **HTML that references local assets** (images, videos via relative paths): put the HTML and all its assets into one folder, name the main file `index.html`, and use the skill's `folder` mode so everything is uploaded together.

3. **Upload** — Invoke the `baas-upload` skill. If the current agent cannot invoke skills, run its script directly:

   ```bash
   python3 <baas-upload-skill-dir>/scripts/upload.py html ./presentation.html
   # or, for decks with local assets:
   python3 <baas-upload-skill-dir>/scripts/upload.py folder ./my-deck
   ```

4. **Share the URL** — Tell the user:
   - The live URL (from the skill output, e.g. `https://baas-store.mayfair-inc.com/r/{root_id}/`)
   - That it works on any device — they can text it, Slack it, email it
   - The link is a permanent resource link; visibility is `public` by default (pass `--visibility private` if the deck is sensitive)

**⚠ Upload gotchas:**

- **Local images/videos must travel with the HTML.** The `html` mode uploads only the single file. If the presentation references local files via `src="..."` or CSS `background-image`, use the folder flow from step 2 instead. **After uploading, verify:** open the returned URL and check that all images load.
- **Re-uploading creates a NEW URL.** Unlike a fixed app domain, every upload generates a new `root_id`/`file_id`. After revisions, upload again and share the newest link — old links keep serving the old version.
- **Verify with the right domain.** Use `https://baas-store.mayfair-inc.com` to check the result; do not use `baas-store.mayfair-inc.net`.
- **Need a fixed, iterable app URL?** That is a Fun app deployment, which is out of scope for `baas-upload` — tell the user this flow only produces one-off permanent resource links.
...
| -------------------------------------------------- | -------------------------------------------------------------------- | ------------------------- |
| [BRAND_GUIDELINES.md](BRAND_GUIDELINES.md)         | SAVANA brand design tokens (colors, type, logo, voice)               | Brand Mode (Phase 2 & 3)  |
| [assets/logo.svg](assets/logo.svg)                 | SAVANA wordmark — inline and recolor per brand rules                 | Brand Mode (Phase 2 & 3)  |
...
| `baas-upload` skill (external)                     | Upload slides to BaaS Storage for instant sharing                    | Phase 6 (sharing)         |
