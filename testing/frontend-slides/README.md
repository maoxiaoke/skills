...
- **Company Brand Mode** — Company-facing decks automatically follow the SAVANA Brand Bible: official palette with WCAG-safe pairings, Montserrat + Poppins, and the logo inlined bottom-left on every slide.
...
cp SKILL.md STYLE_PRESETS.md BRAND_GUIDELINES.md viewport-base.css html-template.md animation-patterns.md ~/.claude/skills/frontend-slides/
cp -R bold-template-pack assets ~/.claude/skills/frontend-slides/
...
| `BRAND_GUIDELINES.md`     | SAVANA brand design tokens     | Brand Mode (company decks) |
| `assets/logo.svg`         | SAVANA wordmark (inline SVG)   | Brand Mode (company decks) |
...
| `baas-upload` skill (external) | Upload slides to BaaS Storage | Phase 6 (sharing)        |
...
### Upload to a Live URL

The skill uploads your slides via the `baas-upload` skill to BaaS Storage and returns a permanent, shareable URL that works on any device — phones, tablets, laptops:

```text
https://baas-store.mayfair-inc.com/r/{root_id}/
```

Single self-contained HTML files upload directly; decks with local assets upload as a folder (with the main file named `index.html`). Requires a `melo-cli login` session. Note: each upload generates a new link — after revisions, share the newest URL.
...
- For URL upload: the `baas-upload` skill + a `melo-cli login` session (BaaS Storage)
